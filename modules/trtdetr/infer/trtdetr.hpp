/**
 * @file trtdetr.hpp
 * @brief TensorRT-DETR 模型推理相关类和结构体的定义
 * @date 2025-06-01
 *
...
#ifdef _MSC_VER
#define TRTDETRAPI __declspec(dllexport)
#else
#define TRTDETRAPI __attribute__((visibility("default")))
#endif
 ...
 */
struct TRTDETRAPI RotatedBox : public Box {
 ...
 */
struct TRTDETRAPI ClassifyRes : public BaseRes {
 ...
 */
struct TRTDETRAPI DetectRes : public BaseRes {
 ...
 */
struct TRTDETRAPI OBBRes : public BaseRes {
 ...
 */
struct TRTDETRAPI SegmentRes : public BaseRes {
 ...
 */
struct TRTDETRAPI PoseRes : public BaseRes {
...
public:
     ...
     * @return InferOption&
     */
    void setConfThresh(float conf_thresh);
...
private:
    ...
    friend class trtdetr::BaseModel;
};
...
class TRTDETRAPI ClassifyModel : public BaseModel {
public:
    // 私有的无参构造函数，仅在 clone 方法中使用
    ClassifyModel();
    ~ClassifyModel();
...
class TRTDETRAPI DetectModel : public BaseModel {
public:
    // 私有的无参构造函数，仅在 clone 方法中使用
    DetectModel();
    ~DetectModel();
...
class TRTDETRAPI OBBModel : public BaseModel {
public:
    // 私有的无参构造函数，仅在 clone 方法中使用
    OBBModel();
    ~OBBModel();
...
class TRTDETRAPI SegmentModel : public BaseModel {
public:
    // 私有的无参构造函数，仅在 clone 方法中使用
    SegmentModel();
    ~SegmentModel();
...
class TRTDETRAPI PoseModel : public BaseModel {
public:
    // 私有的无参构造函数，仅在 clone 方法中使用
    PoseModel();
    ~PoseModel();
