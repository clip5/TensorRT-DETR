/**
 * @file pybind.cpp
 * @brief trtdetr的Python绑定，提供结果、选项和模型模块的包装
 * @date 2025-01-17
 *
 */
...
void bind_result(py::module& m, const std::string& result_name) {
    ...
    if constexpr (std::is_same_v<ResultType, trtdetr::ClassifyRes>) {
        docstring = "A class representing classification results containing predicted classes and their confidence scores.";
    } else if constexpr (std::is_same_v<ResultType, trtdetr::DetectRes>) {
    ...
    } else if constexpr (std::is_same_v<ResultType, trtdetr::OBBRes>) {
    ...
    } else if constexpr (std::is_same_v<ResultType, trtdetr::SegmentRes>) {
    ...
    } else if constexpr (std::is_same_v<ResultType, trtdetr::PoseRes>) {
    ...
    py::class_<ResultType, trtdetr::BaseRes> cls(m, result_name.c_str(), docstring.c_str());

    if constexpr (std::is_same_v<ResultType, trtdetr::DetectRes> ||
                  std::is_same_v<ResultType, trtdetr::OBBRes> ||
                  std::is_same_v<ResultType, trtdetr::SegmentRes> ||
                  std::is_same_v<ResultType, trtdetr::PoseRes>) {
        ...
        if constexpr (std::is_same_v<ResultType, trtdetr::OBBRes>) {
    ...
    if constexpr (std::is_same_v<ResultType, trtdetr::SegmentRes>) {
    ...
    if constexpr (std::is_same_v<ResultType, trtdetr::PoseRes>) {
...
/**
 * @brief 为trtdetr结果类创建Python绑定
 *
 * 该函数负责将trtdetr中的结果相关C++类绑定到Python，使其可以在Python中使用。
 * 主要功能包括：
...
void binding_result_module(py::module& m) {
    m.doc() = "Result module of trtdetr, providing Python bindings for various computer vision result types including classification, object detection, rotated object detection, instance segmentation and pose estimation.";

    // 基础结果类，所有结果类型的基类，提供通用功能和属性
    py::class_<trtdetr::BaseRes>(m, "BaseRes", "A base class for all result types, providing common properties like number of results, class IDs, and confidence scores.")
        // 允许使用Python的len()函数获取结果数量
        .def("__len__", [](const trtdetr::BaseRes& r) { return r.num; }, "Return the number of detected objects via Python's len() function.")
        // 检测对象的类别ID数组
        .def_property_readonly("class_id", [](const trtdetr::BaseRes& r) -> py::array { return py::array(
                                                                                            py::buffer_info(
        ...
        // 检测对象的置信度分数数组
        .def_property_readonly("confidence", [](const trtdetr::BaseRes& r) -> py::array { return py::array(
                                                                                              py::buffer_info(
    ...
    // 绑定各种专门的结果类型
    bind_result<trtdetr::ClassifyRes>(m, "ClassifyRes");  // 分类结果
    bind_result<trtdetr::DetectRes>(m, "DetectRes");      // 目标检测结果(矩形框)
    bind_result<trtdetr::OBBRes>(m, "OBBRes");            // 旋转目标检测结果
    bind_result<trtdetr::SegmentRes>(m, "SegmentRes");    // 实例分割结果(带掩码)
    bind_result<trtdetr::PoseRes>(m, "PoseRes");          // 人体姿态估计结果(带关键点)
}

/**
 * @brief 为trtdetr推理选项创建Python绑定
 *
 * 该函数负责将trtdetr中的InferOption类绑定到Python，提供灵活的推理配置选项。
...
void binding_option_module(py::module& m) {
    ...
    py::class_<trtdetr::InferOption>(m, "InferOption", "A class to configure advanced inference options for trtdetr models, controlling device selection, performance monitoring, and image preprocessing.")
        ...
        // .def("enable_cuda_memory", &trtdetr::InferOption::enableCudaMem, "Inference data already in CUDA memory.")
        // .def("enable_managed_memory", &trtdetr::InferOption::enableManagedMemory, "Enable managed memory for inference.")
        .def("enable_profile", &trtdetr::InferOption::enablePerformanceReport, "Enable performance profile for inference.")
        .def("enable_swap_rb", &trtdetr::InferOption::enableSwapRB, "Enable RGB-to-BGR swap for image input.")
...
/**
 * @brief 将NumPy数组转换为trtdetr::Image对象。
 *
 * 此函数会把一个形状为(height, width, channels)、数据类型为uint8的3D NumPy数组，
 * 转换为trtdetr::Image对象，该对象可用于后续的推理任务。
 ...
 * @return trtdetr::Image 转换后的可用于推理的图像对象。
 * @throws std::invalid_argument 当输入数组不是3维，或者第三维通道数不为3时抛出此异常。
 */
trtdetr::Image PyArray2Image(const py::array_t<uint8_t>& src) {
    py::buffer_info buf = src.request();
    if (buf.ndim != 3 || buf.shape[2] != 3) {
        throw std::invalid_argument("Require rank of array to be 3 with HWC format while converting it to trtdetr::Image.");
    }

    return trtdetr::Image(buf.ptr, buf.shape[1], buf.shape[0], buf.shape[2], buf.strides[0]);
}

/**
 * @brief 为trtdetr模型类创建Python绑定的模板函数
 *
 * 该模板函数提供了一个统一的方式来将各种trtdetr模型类绑定到Python接口，
...
void bind_model(py::module& m, const std::string& model_name) {
    py::class_<ModelType, std::unique_ptr<ModelType>>(m, model_name.c_str(),
                                                      "A trtdetr model class for performing computer vision inference tasks with TensorRT acceleration.")
        .def(py::init<const std::string&, const trtdetr::InferOption&>(),
        ...
        .def("predict", [](ModelType& self, std::vector<py::array>& inputs) {
                std::vector<trtdetr::Image> images;
 ...
 *
 * 该函数负责将trtdetr命名空间下的各类模型（分类、检测、旋转框检测、实例分割、姿态估计）
...
void binding_model_module(py::module& m) {
    m.doc() =
        "Model module of trtdetr, providing TensorRT-accelerated inference for computer vision tasks, "
        "including ClassifyModel, DetectModel, OBBModel, SegmentModel, and PoseModel.";

    bind_model<trtdetr::ClassifyModel>(m, "ClassifyModel");
    bind_model<trtdetr::DetectModel>(m, "DetectModel");
    bind_model<trtdetr::OBBModel>(m, "OBBModel");
    bind_model<trtdetr::SegmentModel>(m, "SegmentModel");
    bind_model<trtdetr::PoseModel>(m, "PoseModel");
}

/**
 * @brief trtdetr Python绑定主模块。
 *
 * 此模块借助Pybind11创建TensorRT-DETR的Python绑定接口，
 * 包含三个子模块：
 ...
 */
PYBIND11_MODULE(py_trtdetr, m) {
    m.doc() = "Python bindings for trtdetr, providing TensorRT-accelerated inference for computer vision tasks.";
