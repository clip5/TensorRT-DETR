...
# 在 nndeploy 工作流中使用 TensorRT-DETR

本示例简要介绍了如何在 nndeploy 的工作流中使用 TensorRT-DETR 工具包实现多类计算机视觉任务，包括**目标检测、实例分割、图像分类、姿态识别及旋转目标检测** 。
...
参考 [TensorRT-DETR 快速编译安装](../../README.md) 文档，执行编译操作与 Python 库打包流程。

> **注意**：编译过程中需确保开启 `BUILD_PYTHON` 选项，随后按照文档步骤完成 Python 库的打包与安装。

## 2. 模型转换

参考 模型导出说明 中的方法，先通过模型 Python 库的命令行工具（CLI）将模型导出为 ONNX 格式，再将 ONNX 模型转换为 TensorRT 引擎。
...
2. 点击 `+` 按钮，导入已下载好的 [`trtdetr_plugin.py`](trtdetr_plugin.py) 文件。
