...
# Using TensorRT-DETR in nndeploy Workflow

This example briefly introduces how to use the TensorRT-DETR toolkit in nndeploy's workflow to implement multi-class computer vision tasks, including **object detection, instance segmentation, image classification, pose estimation, and oriented object detection**.
...
Refer to the [TensorRT-DETR Quick Compilation and Installation](../../README.en.md) document to perform compilation and Python library packaging processes.
...
Refer to the methods in model export instructions. First, export the model to ONNX format using the command-line tool (CLI) of the model's Python library, then convert the ONNX model to a TensorRT engine.
...
2. Click the `+` button to import the downloaded [`trtdetr_plugin.py`](trtdetr_plugin.py) file.
