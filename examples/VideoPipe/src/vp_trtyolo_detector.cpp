#include "vp_trtdetr_detector.h"

namespace vp_nodes {

vp_trtdetr_detector::vp_trtdetr_detector(std::string node_name, std::string model_path, std::string labels_path, int batch, int device_id)
    ...
    detector = std::make_unique<trtdetr::DetectModel>(model_path, option);

    this->initialized();  // Mark node as initialized
}

vp_trtdetr_detector::~vp_trtdetr_detector() {
    // Destructor: Clean up any resources
    deinitialized();  // Mark node as deinitialized
}

void vp_trtdetr_detector::run_infer_combinations(const std::vector<std::shared_ptr<vp_objects::vp_frame_meta>>& frame_meta_with_batch) {
    std::vector<cv::Mat>        mats_to_infer;
    std::vector<trtdetr::Image> images_to_infer;
    ...
    std::transform(mats_to_infer.begin(), mats_to_infer.end(), std::back_inserter(images_to_infer), [](cv::Mat& mat) {
        return trtdetr::Image(mat.data, mat.cols, mat.rows);
    });
    ...
    std::vector<trtdetr::DetectRes> detection_results = detector->predict(images_to_infer);
...
void vp_trtdetr_detector::postprocess(const std::vector<cv::Mat>& raw_outputs, const std::vector<std::shared_ptr<vp_objects::vp_frame_meta>>& frame_meta_with_batch) {
