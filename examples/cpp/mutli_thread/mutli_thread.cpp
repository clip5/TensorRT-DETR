...
void predict(trtdetr::DetectModel* model, int thread_id, const std::vector<std::string>& image_paths) {
    ...
    for (size_t start_idx = 0; start_idx < image_paths.size(); start_idx += batch_size) {
        ...
        std::transform(image_paths.begin() + start_idx,
                       ...
                       std::back_inserter(img_batch), [](const std::string& image_path) {
                           ...
                           return trtdetr::Image{image.data, image.cols, image.rows};
                       });
...
int main(int argc, char** argv) {
    ...
    // 初始化模型选项
    auto option = trtdetr::InferOption();
    option.enableSwapRB();
    option.enablePerformanceReport();

    // 加载模型
    auto model = trtdetr::DetectModel(model_path, option);

    // 克隆模型到多个线程
    std::vector<std::unique_ptr<trtdetr::DetectModel>> models;
