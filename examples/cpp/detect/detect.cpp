...
// 在图像上可视化推理结果
void visualize(cv::Mat& image, const trtdetr::DetectRes& result, const std::vector<std::string>& labels) {
    for (size_t i = 0; i < result.num; ++i) {
        ...
        printf("[%d] %s %.3f, box:%f,%f,%f,%f\n", cls, label.c_str(), score, box.left, box.top, box.right, box.bottom);
        // 绘制矩形和标签
        int      base_line;
...
// 处理单张图像
void process_single_image(const std::string& image_path, const std::string& output_path, trtdetr::DetectModel& model, const std::vector<std::string>& labels) {
...
// 处理一批图像
void process_batch_images(const std::vector<std::string>& image_paths, const std::string& output_path, trtdetr::DetectModel& model, const std::vector<std::string>& labels) {
...
int main(int argc, char** argv) {
    try {
        ...
        auto model = std::make_unique<trtdetr::DetectModel>(engine_path, option);
