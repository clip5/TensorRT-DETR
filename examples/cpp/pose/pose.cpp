...
// 可视化推理结果
void visualize(cv::Mat& image, trtdetr::PoseRes& result, const std::vector<std::string>& labels) {
    // 定义人体关键点连接关系（骨架）
    ...
    for (size_t i = 0; i < result.num; ++i) {
        ...
        auto&       label      = labels[cls - 1];                              // 获取类别对应的标签
...
// 处理单张图像
void process_single_image(const std::string& image_path, const std::string& output_path, trtdetr::PoseModel& model, const std::vector<std::string>& labels) {
...
// 处理一批图像
void process_batch_images(const std::vector<std::string>& image_paths, const std::string& output_path, trtdetr::PoseModel& model, const std::vector<std::string>& labels) {
...
int main(int argc, char** argv) {
    try {
        ...
        option.enableSwapRB();
        option.setConfThresh(0.4);

        if (!fs::is_regular_file(input_path)) {
            option.enablePerformanceReport();
        }

        auto model = std::make_unique<trtdetr::PoseModel>(engine_path, option);
